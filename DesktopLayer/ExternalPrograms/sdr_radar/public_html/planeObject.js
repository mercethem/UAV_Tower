var planeObject = {
	...
	funcUpdateData	: function(data){
			...
			// Uçak reapable olduğunda çemberleri temizle
			if (this.reapable) {
				this.funcClearCircles();
			}
		},
	...
	// Uçağın konumu güncellendiğinde çemberi güncelleyen fonksiyon
	funcUpdateCircles: function() {
		if (!this.vPosition) return;
		...
		// Çemberi dizinin başına ekle
		this.circles.unshift(newCircle);

		// Eski çemberlerin opaklığını azalt ve 5 saniyede bir yok et
		for (var i = 1; i < this.circles.length; i++) {
			var circle = this.circles[i];
			var currentOpacity = 0.8 - (i * 0.1); // Her çember için azalan opaklık
			
			circle.setOptions({
				strokeOpacity: currentOpacity,
				fillOpacity: currentOpacity / 8
			});

			// 5 saniye sonra çemberi kaldır
			...
			})(circle, i), 5000); // 5000 ms = 5 saniye
		}

		// Maksimum çember sayısını aşıyorsa en eski çemberi kaldır
		if (this.circles.length > this.MAX_CIRCLES) {
			var oldCircle = this.circles.pop();
			oldCircle.setMap(null);
		}
	},

	// Tüm çemberleri temizleyen fonksiyon
	funcClearCircles: function() {
	...
	// Çember özelliklerini tanımla
	circles: [],
	MAX_CIRCLES: 10, // Maksimum çember sayısını artırdık
	CIRCLE_RADIUS: 2000
};
